...
programName="arduinopc"
programLongName="arduinopc"
...
dependancy="$globalLibDir/$dependancyLibName"
...
dependancyResult=$(ls "$globalLibDir" | grep "$dependancyRootName")
...
suLinkFile "$sourceDir/C++/arduino/include/arduino.h" "$globalIncludeDir" || { echo "Could not link file, bailing out"; exit 1; }
suLinkFile "$buildDir/lib$programName.so" "$globalLibDir"  || { echo "Could not link file, bailing out"; exit 1; }
suLinkFile "$buildDir/lib$programName.a" "$globalLibDir"  || { echo "Could not link file, bailing out"; exit 1; }
