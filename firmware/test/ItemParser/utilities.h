...
namespace Utilities
{
    ...
    void genericSplitCast(const char *str, const char *delimiter, First *&first, Args* ... arguments)
    {
        ...
        if (strlen(temp) == 0) {
            first = nullptr;
        } else if (is_integral<First>::value) {
            (void)stripNonDigits(temp);
            if (strlen(temp) == 0) {
                first = nullptr;
            } else {
                std::cout << "First was interpreted as an integer (first = " << temp << ")" << std::endl;
            ...
            std::cout << "First was interpreted as a floating point number (first = " << temp << ")" << std::endl;
        ...
        } else if (is_c_str<decltype(first)>::value) {
            std::cout << "First was interpreted as a c string (first = " << temp << ")" << std::endl;
            for (size_t i = 0; i < strlen(temp); i++) {
                first[i] = temp[i];
            }
            first[strlen(temp)] = '\0';
        } else {
            std::cout << "First was not interpreted" << std::endl;
            first = nullptr;
        }
